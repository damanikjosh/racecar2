  ...
  const double steer_pos)
{
  ...
  // Prevent division by zero -josh
  double vel_r = right_traction_wheel_vel * wheel_radius_ * 2 * wheelbase_ /
                 (2 * wheelbase_ + wheel_track_ * std::tan(steer_pos));
  double vel_l = left_traction_wheel_vel * wheel_radius_ * 2 * wheelbase_ /
                 (2 * wheelbase_ - wheel_track_ * std::tan(steer_pos));
  return (vel_r + vel_l) * 0.5;
}

std::tuple<double, double> SteeringOdometry::get_linear_velocity_and_steer_pos_quadruple_traction_axle(
  const double right_rear_traction_wheel_vel, const double left_rear_traction_wheel_vel,
  const double right_front_traction_wheel_vel, const double left_front_traction_wheel_vel,
  const double right_steer_pos, const double left_steer_pos)
{
  ...
  double steer_pos = (right_steer_pos_est + left_steer_pos_est) * 0.5;

  // Prevent division by zero -josh
  double vel_rr = right_rear_traction_wheel_vel * wheel_radius_ * 2 * wheelbase_ /
                  (2 * wheelbase_ + wheel_track_ * std::tan(steer_pos));
  double vel_rl = left_rear_traction_wheel_vel * wheel_radius_ * 2 * wheelbase_ /
                  (2 * wheelbase_ - wheel_track_ * std::tan(steer_pos));
  double lin_vel = (vel_rr + vel_rl) * 0.5;
  return std::make_tuple(lin_vel, steer_pos);
}
...
bool SteeringOdometry::update_from_velocity(
    ...
    const double right_steer_pos, const double left_steer_pos, const double dt)
{
  auto [linear_velocity, steer_pos] = get_linear_velocity_and_steer_pos_quadruple_traction_axle(
    right_rear_traction_wheel_vel, left_rear_traction_wheel_vel, right_front_traction_wheel_vel,
    left_front_traction_wheel_vel, right_steer_pos, left_steer_pos);
...
std::tuple<std::vector<double>, std::vector<double>> SteeringOdometry::get_commands_ackermann(
  const double v_b, const double steer_bz)
{
  // wheel speed
  double Ws = v_b / wheel_radius_;

  std::vector<double> traction_commands;
  std::vector<double> steering_commands;

  double tan_steer_bz = std::tan(steer_bz);

  double phi_l = std::atan2(
    wheelbase_ * tan_steer_bz,
    wheelbase_ - wheel_track_ / 2 * tan_steer_bz);
  double phi_r = std::atan2(
    wheelbase_ * tan_steer_bz,
    wheelbase_ + wheel_track_ / 2 * tan_steer_bz);

  // Calculate differential wheel speeds
  double Wlr = Ws * (1 - wheel_track_ / (2 * wheelbase_) * tan_steer_bz);
  double Wrr = Ws * (1 + wheel_track_ / (2 * wheelbase_) * tan_steer_bz);

  double wb_tan = wheelbase_ * tan_steer_bz;
  double wt_tan = wheel_track_ / 2 * tan_steer_bz;

  double sqrt_neg = std::sqrt(std::pow(wb_tan, 2) + std::pow(wheelbase_ - wt_tan, 2));
  double sqrt_pot = std::sqrt(std::pow(wb_tan, 2) + std::pow(wheelbase_ + wt_tan, 2));

  double del_Wf = Ws * (sqrt_neg - sqrt_pot) / (sqrt_neg + sqrt_pot);
  double Wlf = Ws + del_Wf;
  double Wrf = Ws - del_Wf;

  traction_commands = {Wrr, Wlr, Wrf, Wlf};
  steering_commands = {phi_r, phi_l};
...
{
  linear_acc_ = RollingMeanAccumulator<double>(velocity_rolling_window_size_);
  angular_acc_ = RollingMeanAccumulator<double>(velocity_rolling_window_size_);
}
