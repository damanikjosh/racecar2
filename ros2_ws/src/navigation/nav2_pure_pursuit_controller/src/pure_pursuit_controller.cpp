  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  auto goal_pose_it = std::find_if(
    transformed_plan.poses.begin(), transformed_plan.poses.end(), [&](const auto & ps) {
      return hypot(ps.pose.position.x, ps.pose.position.y) >= std::max(0.35 * (velocity.linear.x * velocity.linear.x), 1.0);
    });
  ...
  if (goal_pose.position.x > 0) {
    auto curvature = goal_pose.position.y /
      (goal_pose.position.x * goal_pose.position.x + goal_pose.position.y * goal_pose.position.y);
    // Scale down the linear velocity if the curvature is too high
    linear_vel = 1.0 * desired_linear_vel_ / (1.0 + 4.0 * abs(goal_pose.position.y));
